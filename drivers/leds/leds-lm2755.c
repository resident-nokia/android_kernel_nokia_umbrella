...
#include <linux/of_gpio.h> //SW-PRODUCTION-JH-Enable GPIO+
...
//SW-PRODUCTION-JH-Enable GPIO+[
int g_enable_gpio;

void lm2755_power_on(bool state)
{
    int retval = 0, value = 0;

	if (!gpio_is_valid(g_enable_gpio))
	{
		printk("%s: could not acquire enable gpio (err=%d)\n",__func__, retval);
		return;
	}
	else
	{
        retval = gpio_request(g_enable_gpio, "lm2755Enable");
        if (retval)
		{
            printk("%s: Failed to get attn gpio %d. Code: %d.",__func__, g_enable_gpio, retval);
            return;
        }
		else
		{
			value = state ? 1 : 0;
			retval = gpio_direction_output(g_enable_gpio, value);
			if (retval) {
				pr_err("%s: Failed to setup gpio %d. rc: %d.",__func__, g_enable_gpio, retval);
				gpio_free(g_enable_gpio);
			}
			else
			{
				printk("%s: lm2755 gpio_direction_output(%d, %d) success\n", __func__, g_enable_gpio, value);
				udelay(30);
			}
		}
	}
...
//SW-PRODUCTION-JH-Enable GPIO+[
	g_enable_gpio = of_get_named_gpio(np, "enable-gpio", 0);
	printk("Add lm2755_probe get gpio => %d\n",(int)g_enable_gpio);
	pdata->enable = lm2755_power_on;
//SW-PRODUCTION-JH-Enable GPIO+]
